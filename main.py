...
def search():
    ...
    # Generate structured recommendations for influential papers
    if len(articles) > 0:
        recommendations = generate_paper_recommendations(query, articles)
        
        # For each recommended paper, find a related paper from arXiv
        for paper_type in recommendations:
            if paper_type != "query" and recommendations[paper_type]:
                paper = recommendations[paper_type]
                # Use the paper title to find related papers
                related_title = paper.get('title', '')
                related_query = f"{related_title} {query}"
                related_papers = search_articles(related_query, max_results=2)
                
                # Add the first related paper that's not already in our results
                for related_paper in related_papers:
                    if related_paper['url'] not in [a['url'] for a in articles]:
                        related_paper['is_related_to'] = paper['title']
                        articles.append(related_paper)
                        # Add to DB if not present
                        if related_paper['url'] not in articles_db:
                            add_article_to_db(related_paper)
                        break
    else:
        recommendations = {
            "query": query,
    ...
    # Return citation priority flag to frontend along with recommendations
    return jsonify({
        "papers": articles,
...
def generate_paper_recommendations(query, articles):
    ...
    Parameters:
        query: The user's search query
        articles: List of articles from the search results
    ...
    # Create a prompt for GPT to analyze the papers
    papers_text = ""
    for i, article in enumerate(articles[:min(len(articles), 10)]):  # Limit to 10 papers to avoid token limits
    ...
    prompt = f"""
    Based on the following papers related to the query "{query}", identify:
    
    1. The most likely to be highly cited or influential paper
    2. The most relevant paper to the query
    3. The most recent significant paper
    
    For each category, provide your reasoning based on the paper's content, methodology, and potential impact.
    ...
    {{
        "most_cited": {{
            "index": paper_index,
            "title": "paper title",
            "reasoning": "explanation of why this paper is likely to be highly cited"
        }},
        "most_relevant": {{
            "index": paper_index,
            "title": "paper title",
            "reasoning": "explanation of why this paper is most relevant to the query"
        }},
        "most_recent": {{
            "index": paper_index,
            "title": "paper title",
            "reasoning": "explanation of why this recent paper is significant"
        }}
    }}
    ...
    try:
        ...
        # Map the recommendations to the actual papers
        for paper_type in ["most_cited", "most_relevant", "most_recent"]:
            if paper_type in analysis and analysis[paper_type]:
                paper_index = analysis[paper_type].get("index", 0) - 1  # Convert to 0-based index
                if 0 <= paper_index < len(articles):
                    paper = articles[paper_index].copy()
                    paper["recommendation_reason"] = analysis[paper_type].get("reasoning", "")
                    recommendations[paper_type] = paper
...
def analyze_papers_batch():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system",
             "content": "You are a research assistant that analyzes academic papers in computer science and AI."},
...
def compare_papers():
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are a research assistant that compares academic papers in computer science and AI."},
...
def expand_keywords():
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Identify 3-5 related keywords or phrases that could enhance the search results for the given query. Return only the JSON array of additional keywords."},
                {"role": "user", "content": query}
            ],
            response_format={"type": "json_object"}
        )
...
def generate_questions():
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Given the user's search query, generate 2-3 questions to assess their understanding of key concepts related to the query. Each question should directly ask about their familiarity with a concept. Return the response as a JSON array of objects with 'question' property."},
                {"role": "user", "content": query}
            ],
            response_format={"type": "json_object"}
        )
...
def generate_knowledge_insights():
    try:
        ...
        for topic, data in user_profile.get('knowledgeAreas', {}).items():
            ...
            knowledge_areas_data.append(
                f"- {topic}: Understanding level {understanding_level}/10, Interaction count: {count}")
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are an AI education advisor specializing in machine learning and AI education."},
...
def get_paper_metadata():
    ...
    try:
        ...
        search = arxiv.Search(id_list=[arxiv_id], max_results=10)
